...
bool LCDIC2::begin()
{
  ...
  return (_height && _width) && send(0b110000) && wait(4100) && send(0b110000) && wait(100) && send(0b110000) && wait(100) && send(0b100000) && wait(100) && write(LCDIC2_FUNCTION | LCDIC2_BITS_4 | (_height > 1) << 3 | (_font & (_height == 1)) << 2) && write(LCDIC2_DISPLAY | _display << 2 | _cursor << 1 | _blink) && write(0b1) && write(LCDIC2_MODE | _gain << 1 | _shift);
}
...
bool LCDIC2::getGlyph(uint8_t glyph, uint8_t *&buffer)
{
  ...
  {
    if (!write(LCDIC2_CGRAM | (glyph << (3 + _font)) | i))
      return false;
...
uint8_t LCDIC2::print(int data) { return LCDIC2::print((String)data); }
uint8_t LCDIC2::print(unsigned int data) { return LCDIC2::print((String)data); }
uint8_t LCDIC2::print(float data) { return LCDIC2::print((String)data); }
uint8_t LCDIC2::print(long data) { return LCDIC2::print((String)data); }
uint8_t LCDIC2::print(unsigned long data) { return LCDIC2::print((String)data); }
uint8_t LCDIC2::print(double data) { return LCDIC2::print((String)data); }
uint8_t LCDIC2::print(const char data[]) { return LCDIC2::print((String)data); }
...
bool LCDIC2::setGlyph(uint8_t glyph, uint8_t buffer[])
{
  uint8_t x, y;
  getCursor(x, y);
  if (!write(LCDIC2_CGRAM | (glyph << (3 + _font))))
    return false;
  for (uint8_t i = 0; i < 1 << (3 + _font); i++)
    if (!write((i > 7 + 2 * _font ? 0 : buffer[i]), 0b1))
      return false;
  return setCursor(x, y);
}
